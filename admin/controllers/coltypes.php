...
class Coltypes extends CRUD_Controller {
	...
	public function index($page=1){
		...
		$vdata['title'] = "分类管理";
		$vdata['list'] = $this->model->get_ctypes_path($this->cid,0,$this->name);
	...
	public function get_ctypes_path_ajax($cid=false,$fid=0,$name='ctype')
	{
		...
		if ($cid AND is_numeric($cid)) {
			$data = $this->model->get_ctypes_path($cid,$fid,$name);
		}
...
// 用户组织化分类列表
function tree_ctype($list=false, $padding = array()){
	if (!$list) {
		return "";
	}
