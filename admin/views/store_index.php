...
<script>
store = {
    url_del: "<?php echo site_urlc('store/delete'); ?>"
    ,url_audit: "<?php echo site_urlc('store/audit'); ?>"
    ,url_flag: "<?php echo site_urlc('store/flag'); ?>"
    ,url_sortid: "<?php echo site_urlc('store/sortid'); ?>"
};

$(function(){
    ui.fancybox_img();
    ui.btn_delete(store.url_del);     // 删除
    ui.btn_audit(store.url_audit);    // 审核
    ui.btn_flag(store.url_flag);      // 推荐
    ui.sortable(store.url_sortid);    // 排序
});
</script>
