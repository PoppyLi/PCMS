...
var media = {
    ...
    //通过FID处理删除
        ...
        .done(function(data){
            ...
            if (data.status == 0) {
                ...
                var input_name =$('.js-goal[data-id='+id+']').parents('.js-img-list-f').prev().find('input[type=hidden].form-upload').attr('name');
                // 整理界面
                media.delete2(id,input_name);
                // 额外删除的效果的处理
                if (typeof media_delete!= 'undefined' && media_delete instanceof Function) {
                    media_delete(id,input_name);
                }
            }
            $.pnotify({
                title: '删除文件'
    ...
    ,delete2:function (id,input_name){
        // 对单个存储的单独处理
    ...
    // 上传结束后整理表单
    ,done:function(tdata,input_name){
        // 单个
        if ($('input[name='+input_name+']').attr('data-more') == 0) {
            ...
            // 多个
            var html = template.render('tpl-photos-show', tdata);
            $('#js-'+input_name+'-show').append(html);
    ...
    // 加载默认的图片
    ,show:function(tdata,input_name){
        if (tdata && typeof tdata != 'undefined' && !tools.isEmpty(tdata)) {
            ...
            if (tdata instanceof Array) {
                data = {upload_url:UPLOAD_URL,static_url:STATIC_URL,list:tdata};
                html = template.render('tpl-photos-show', data);
            }else{
                data = {upload_url:UPLOAD_URL,static_url:STATIC_URL,it:tdata};
    ...
    ,get_day_list:function(day,domf,tpl){
        $.ajax({
            type: "GET"
            ,url:UPLOADDO_URL+'get_day_ajax'
            ,data:{dt:day,nocache:Math.random()}
            ,dataType:'json'
        })
...
// •   memory_limit
    ...
    $('input[type="file"].fileupload').fileupload({
        ...
        ,done:function (e, data) {
            ...
            // 对应的表单name
            var input_name = $(this).parent().next('input[type=hidden]').attr('name');

            // UI 整理
            media.done(tdata,input_name);
            // 上传后额外UI输出执行函数
            if (typeof media_done != 'undefined' && media_done instanceof Function) {
                media_done(tdata,input_name);
            }

            }else{
                $.pnotify({
                    title: '上传失败'
                    ,text: '您的文件不被允许上传，请询问管理员！'
                    ,type: 'error'
                    ,animation: 'fade'
                });
            };
        ...
        ,progressall:function (e, data) {
            ...
            $('#upload-modal .progress .progress-bar').css('width',progress + '%');
