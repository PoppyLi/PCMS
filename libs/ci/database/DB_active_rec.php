...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function where($key, $value = NULL, $escape = TRUE)
	{
		if (!is_array($key)) {
			...
			// 参数限定数组
			foreach ($key as $k => $w) {
				preg_match("/^or|^in|^or_in|^not_in|^or_not_in|^like|^not_like|^not_like|^or_like|^or_not_like|^having|^or_having/ix",$k,$m);
				$str = isset($m[0]) ? $m[0]:'';
				switch ($str) {
					case 'or':
					...
					case 'like':
						...
						return $this->_like($w[0], $w[1], 'AND ', $side);
						break;
					case 'not_like':
						$side = isset($w[2]) ? $w[2]:'both';
						return $this->_like($w[0], $w[1], 'AND ',$side, 'NOT');
						break;
					case 'or_like':
						$side = isset($w[2]) ? $w[2]:'both';
						return $this->_like($w[0], $w[1], 'OR ',$side);
						break;
					case 'or_not_like':
						$side = isset($w[2]) ? $w[2]:'both';
						return $this->_like($w[0], $w[1], 'OR ',$side, 'NOT');
						break;
					case 'having':
						return $this->_having($w[0], $w[1], 'AND ', $escape);
						break;
					case 'or_having':
						return $this->_having($w[0], $w[1], 'OR ', $escape);
						break;
					default:
